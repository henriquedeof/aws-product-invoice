    ...
    public InvoiceAppStack(final Construct scope, final String id, final StackProps props) {
        ...
        SnsTopic s3InvoiceTopic = SnsTopic.Builder.create(
                Topic.Builder.create(this, "S3InvoiceTopic")
                        .topicName("s3-invoice-events-xpto")
                        .build()
        )
        .build();

        this.bucket = Bucket.Builder.create(this, "S301").bucketName("pcs01-invoice").removalPolicy(RemovalPolicy.DESTROY).build();
        ...
        Queue s3InvoiceDlq = Queue.Builder.create(this, "S3InvoiceDql").queueName("s3-invoice-events-dlq").build();
        DeadLetterQueue deadLetterQueue = DeadLetterQueue.builder().queue(s3InvoiceDlq).maxReceiveCount(3).build();


        this.s3InvoiceQueue = Queue.Builder.create(this, "S3invoiceQueue").queueName("s3-invoice-events").deadLetterQueue(deadLetterQueue).build();
