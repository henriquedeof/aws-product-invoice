...
public class SnsStack extends Stack {
    ...
    public SnsStack(final Construct scope, final String id, final StackProps props) {
        ...
        productEventsTopic = SnsTopic.Builder.create(
                Topic.Builder.create(this, "ProductEventsTopic").topicName("product-events").build()
        ).build();

        productEventsTopic.getTopic().addSubscription(EmailSubscription.Builder.create("henriquedeof@hotmail.com").json(true).build());
    }
